...
if __name__ == '__main__':
    ...
    try:
        while True:
            info = wait_for_print_job_info(timeout=0.25, printer_name=printer_to_be_used)
            if not info:
                continue
            for nd in info:
                job_id, key, value = nd
                job_key = 'jobid_' + str(job_id)
                if key == 'job_status':
                    status_string = job_status_string(value)
                    value = '%#010x (%s)' % (value, status_string)

                    # PRINTING, RETAINED
                    if value.startswith('0x00002010'):
                        try:
                            job_info = win32print.GetJob(printer, job_id, 1)
                            jobs[job_key] = job_info['pDocument']
                            print('Printing started: ', jobs[job_key])
                        except:
                            print('Job Finished')
                    elif value.startswith('0x00001000'):
                        # move completed file to processed
                        os.rename(os.path.join(INPUTDIR, jobs[job_key]), os.path.join(OUTPUTDIR, jobs[job_key]))
                        print('Printing finished: ', jobs[job_key])
                        del jobs[job_key]
